...
def load_spec_from_file(spec_file: str) -> dict[str, Any]:
    ...
    if not os.path.exists(spec_file):
        ...
        if os.path.exists(flows_path):
            ...
            flows_path_py = os.path.join(
                "src", "telematry_cfs", "flows", f"{spec_file}.py"
            )
...
def run_flow(
    ...
    if telemetry:
        telemetry_manager = TelemetryManager(base_path=output_dir, flow_name=flow_name)
    ...
    try:
        ...
        logger.info(
            "Energy compilation completed successfully",
            extra={"has_compile_report": compiled.compile_report is not None},
        )
        ...
        if telemetry_manager:
            ...
            logger.debug(
                "Telemetry data flushed", extra={"run_path": telemetry_manager.run_path}
            )
...
def tune_flow(
    ...
    print(f"Flow: {flow_name}")
    print(
        f"Gap Dial tuning: target_gap={target_gap}, adaptation_rate={adaptation_rate}"
    )
...
def main():
    ...
    run_parser.add_argument(
        "spec_file", help="Python file containing the flow specification"
    )
    run_parser.add_argument("--output", "-o", help="Output directory for results")
    run_parser.add_argument(
        "--no-telemetry", action="store_true", help="Disable telemetry logging"
    )
    run_parser.add_argument(
        "--gap-dial",
        action="store_true",
        help="Enable Gap Dial auto-tuner for in-run parameter optimization",
    )
    ...
    tune_parser = subparsers.add_parser(
        "tune", help="Run Gap Dial parameter optimization"
    )
    ...
    tune_parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Target spectral gap (default: 0.1)",
    )
    tune_parser.add_argument(
        "--adaptation-rate",
        type=float,
        default=0.1,
        help="Parameter adaptation rate (default: 0.1)",
    )
    tune_parser.add_argument(
        "--iterations",
        type=int,
