...
class FlowPolicy(BaseModel):
    ...
    def validate_preconditioner_name(cls, v):
        ...
        if v is not None and not v.replace('_', '').replace('-', '').isalnum():
            raise ValueError("preconditioner name must contain only alphanumeric characters, underscores, and hyphens")
        return v
...
class MultiscaleSchedule(BaseModel):
    ...
    levels: int = Field(
        ...
        ge=1,
        le=20,
        description="Number of multiscale levels (1-20)"
    )
    ...
    def validate_activate_rule(cls, v):
        ...
        if not has_operator and v != 'level_complete':
            raise ValueError("activate_rule must contain a comparison operator or be 'level_complete'")
