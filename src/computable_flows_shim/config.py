...
def get_dtype(name: str = "default") -> jnp.dtype:
    ...
    if name not in DTYPE_POLICY:
        raise ValueError(
            f"Unknown dtype '{name}'. Available: {list(DTYPE_POLICY.keys())}"
        )
    return DTYPE_POLICY[name]


def enforce_dtype(array: jax.Array, dtype: str | None = None) -> jax.Array:
...
def validate_dtype_consistency():
    ...
    assert (
        DEFAULT_DTYPE == jnp.float64
    ), "Default dtype should be float64 for numerical stability"
    assert (
        DEFAULT_COMPLEX_DTYPE == jnp.complex128
    ), "Default complex dtype should be complex128"
    return True
