...
class TransformOp:
    ...
    def __post_init__(self):
        ...
        if not JAXWT_AVAILABLE:
            raise ImportError(
                "jaxwt required for TransformOp. Install with: pip install jaxwt"
            )
    ...
    def _create_forward_fn(self) -> Callable[[jnp.ndarray], Any]:
        ...
        if self.ndim == 1:
            ...
            def forward_2d(
                x: jnp.ndarray,
            ) -> Any:  # jaxwt.wavedec2 has complex return type
    ...
    def _create_inverse_fn(self) -> Callable[[Any], jnp.ndarray]:
        ...
        if self.ndim == 1:
            ...
            def inverse_2d(
                coeffs: Any,
            ) -> jnp.ndarray:  # Accept complex coefficient structure
...
def make_jaxwt_transform(wavelet: str, levels: int, ndim: int = 1) -> TransformOp:
    """
...
def make_transform(wavelet: str, levels: int, ndim: int = 1) -> TransformOp:
    """
