...
class TestConfiguration:
    ...
    def test_configure_jax_environment_64bit_disabled(self, mock_config_update):
        ...
        assert not any(
            call[0][0] == "jax_enable_x64" for call in mock_config_update.call_args_list
        )
...
class TestValidation:
    ...
    def test_validate_xla_flags_conflicting_fast_math(self):
        ...
        conflicting_flags = (
            "--xla_enable_fast_math=true --xla_cpu_enable_fast_math=false"
        )
        ...
        try:
            for name, dtype in DTYPE_POLICY.items():
                ...
                try:
                    ...
                    pytest.fail(
                        f"Invalid dtype in policy: {name} -> {dtype}, error: {e}"
                    )
        finally:
