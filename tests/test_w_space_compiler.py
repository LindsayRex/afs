...
def test_wavelet_l1_prox_in_W_space():
    ...
    assert hasattr(compiled, 'g_prox_in_W'), "CompiledEnergy should have g_prox_in_W method"
    ...
    for i, (orig, prox) in enumerate(zip(coeffs, result_coeffs)):
...
def test_W_space_prox_mathematical_properties():
    ...
    for i, (p1, p2, c1, c2) in enumerate(zip(prox1, prox2, coeffs1, coeffs2)):
        diff_prox = jnp.linalg.norm(p1 - p2)
        diff_coeffs = jnp.linalg.norm(c1 - c2)
        assert diff_prox <= diff_coeffs + 1e-6, f"Non-expansiveness failed for coeff array {i}"
    ...
    for i, (pz, cz) in enumerate(zip(prox_zero, zero_coeffs)):
        assert jnp.allclose(pz, cz, atol=1e-6), f"Fixed point failed for coeff array {i}"
