...
class TestLensProbeContract:
    ...
    def test_compressibility_metric_calculation(self, sample_data_1d, candidate_transforms_1d):
    ...
    def test_reconstruction_error_calculation(self, sample_data_1d, candidate_transforms_1d):
        """Test that reconstruction error is calculated correctly."""
        from computable_flows_shim.multi.lens_probe import calculate_reconstruction_error
    ...
    def test_lens_selection_by_reconstruction_error(self, sample_data_1d, candidate_transforms_1d):
        ...
        selected_error = candidate_results[selected]['reconstruction_error']['relative_error']

        for name, results in candidate_results.items():
            if name != selected:
                assert results['reconstruction_error']['relative_error'] >= selected_error
    ...
    def test_lens_probe_2d_support(self, sample_data_2d, candidate_transforms_2d):
        ...
        probe_results = run_lens_probe(
            data=sample_data_2d,
            candidates=candidate_transforms_2d,
            target_sparsity=0.9
        )
    ...
    def test_probe_result_consistency(self, sample_data_1d, candidate_transforms_1d):
        ...
        results1 = run_lens_probe(data=sample_data_1d, candidates=candidate_transforms_1d)
        results2 = run_lens_probe(data=sample_data_1d, candidates=candidate_transforms_1d)
