...
from computable_flows_shim.multi.transform_op import (
    make_jaxwt_transform,
    make_transform,
)
...
class TestTransformOpContract:
    ...
    def test_1d_round_trip_accuracy(self, sample_1d_signal, float_dtype):
        ...
        assert (
            error < tolerance
        ), f"Round-trip error too large: {error} (dtype: {float_dtype})"
    ...
    def test_coefficient_structure_preservation(self, sample_1d_signal):
        ...
        for c1, c2 in zip(coeffs1, coeffs2, strict=False):
    ...
    def test_frame_constant_mathematical_property(self, sample_1d_signal):
        ...
        if transform.frame == "tight":
            ...
            assert (
                abs(ratio - transform.c) < 1e-6
            ), f"Frame constant violation: {ratio} vs {transform.c}"
